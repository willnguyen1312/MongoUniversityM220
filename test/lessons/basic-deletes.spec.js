describe("Basic Deletes", async () => {
   ...
   * the "D" in Create, Read, Update, Delete
   *
   * The first method we'll talk about is deleteOne. Before we do this, we need to create a collection and a
  ...
  test("insertMany", async () => {
    /**
     * First, let's insert documents into this collection so that we can delete them during this lesson.
     */
  ...
  test("deleteOne", async () => {
    // Now let's try to delete the document using deleteOne()
    ...
    console.log(countDocumentsBefore)
    ...
    // To check that our delete was successful, we can check that result.n is equal to 1. This is an object returned
    // by the MongoDB Javascript driver which will tell the client if a delete has been successful or not.
    // Lets test to see if the delete was successful.

    expect(deleteDocument.result.n).toBe(1)

    // Great. That's what we expected!

    // If we now count the number of documents remaining in our collection, we should see that there is one less than we inserted
  ...
  test("deletewithPredicate", async () => {
    /**
    ...
    // To check that our delete was successful, we'll check the result.n object again.

    expect(deleteDocument.result.n).toBe(1)

    // If we now count the number of documents remaining in our we should see that there is one less than we inserted
  ...
  // Next, we will use the deleteMany operator. As the name suggests, this operator allows us to
  // delete many documents, which match a give filter in our delete statement. Let's check it out:

  test("deleteMany", async () => {
    // let's return all documents that are left in the collection to see what we can delete
    // before we try to delete it using the deleteMany operator.

    let result = await videoGames.find({})

    console.log(result)

    // Now let's try to delete multiple documents using deleteMany().
    // To do this, we will need to specify a filter for the delete statment that will match multiple documents.

    let deleteManyDocs = await videoGames.deleteMany({ title: "Mega Man" })

    // This will delete all documents that have a title that matched "Mega Man"
    // To check that our delete was successful, we can check what the value of deleteMayDocs.result.n is equal to.
    // In this case, we expect it to be 8.

    console.log(deleteManyDocs.result.n)

    expect(deleteManyDocs.result.n).toBe(8)
  })
})
